...
def parse_metadata(text: Optional[str]) -> dict:
    ...
    if not seasons and not episodes:
        se = extract_seasons_episodes(raw)
        seasons = se["seasons"]
...
def extract_seasons_episodes(text: str) -> dict:
    """Regex season/episode extraction when PTT lists are empty."""
    norm = _normalize_for_match(text or "")
    m = re.search(
        r"(?i)\bS(\d{1,2})\s*E(?:P)?(\d{1,3})(?:\s*[-–]\s*E?(?:P)?(\d{1,3}))?\b",
        norm,
    )
    if m:
        ...
        return {"seasons": [int(m.group(1))], "episodes": episodes}
    m = re.search(r"(?i)\b(\d{1,2})x(\d{1,3})\b", norm)
    if m:
        return {"seasons": [int(m.group(1))], "episodes": [int(m.group(2))]}
    m = re.search(
        r"(?i)\bSeason\s*(\d{1,2})(?:\s*(?:Episode|Ep\.?|E)\s*(\d{1,3}))?\b",
        norm,
    )
    if m:
        return {
            "seasons": [int(m.group(1))],
            "episodes": [int(m.group(2))] if m.group(2) else [],
        }
    m = re.search(r"(?i)\bS(\d{1,2})\b", norm)
    if m:
        return {"seasons": [int(m.group(1))], "episodes": []}
...
def group_key_from_metadata(meta: dict) -> Optional[str]:
    ...
    Movies: title + year (quality / codec ignored).
    Series: title + season (episodes, Combined/Complete, quality ignored).
            A new season is a new group. Same season's E01/E02/Combined share one key.
    Year is omitted from series keys when a season is present so
    "Mirzapur S01" and "Mirzapur (2018) S01 E01" stay in the same group.
    """
    ...
    for s in meta.get("seasons") or []:
        try:
            ...
            continue
    if season_ids:
        ss = "-".join(str(s) for s in sorted(set(season_ids)))
        return f"series:{title}:s{ss}"
    if year_s:
...
def get_content_group_key(message: Any) -> Optional[str]:
    """Logical movie/series group for title-change stickers (quality ignored)."""
    return group_key_from_metadata(parse_message_metadata(message))


def title_changed(last_key: Optional[str], new_key: Optional[str]) -> bool:
    """True only when a movie/series group starts or the title/season changes.

    Unknown / untitled messages never trigger a sticker and do not reset
    the current group, so random text between two Thor files is ignored.
    """
    if not new_key:
        return False
...
def plan_sticker_stream(captions: Iterable[str]) -> list[dict]:
    """Simulate the auto-forward sticker stream for a list of captions.

    A sticker is inserted immediately BEFORE the first file of each new
    movie title or series season. Quality variants and same-season episodes
    share the group. The same title returning after a different title gets
    a new sticker.
    """
    ...
    for raw in captions:
        ...
        if not cap:
            continue
        meta = parse_metadata(cap)
        key = group_key_from_metadata(meta)
        if title_changed(last, key):
            out.append({
                "kind": "sticker",
                "key": key,
